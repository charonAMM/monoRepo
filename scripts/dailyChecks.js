        ...
        try {
            ...
            if(thisUTXO.amount > 0 && toFixedHex(events[i].args._commitment) == toFixedHex(thisUTXO.getCommitment(poseidon))){
                try{
                    ...
                    console.log(await charonInstance.isSpent(myNullifier))
                    if(!await charonInstance.isSpent(myNullifier)){
                        console.log("here", thisUTXO.amount)
                        myAmount += parseInt(thisUTXO.amount);
                        console.log("here", myAmount)
                    }
                }catch{
...
async function runChecks() {
    ...
    let sWallet = new ethers.Wallet(process.env.MAINPK, oprovider);
    let sepSigner = sWallet.provider.getSigner(sWallet.address)
    ...
    let wallet = new ethers.Wallet(process.env.PK, gprovider);
...
console.log("my private balance OPTIMISM CHD", await getPrivateBalance(sepoliaCharon,myKeypair,10))
//console.log("my private balance GNOSIS CHD", await getPrivateBalance(chiadoCharon,myKeypair,100))
console.log("my private balance POLYGON CHD", await getPrivateBalance(mumbaiCharon,myKeypair,137))
